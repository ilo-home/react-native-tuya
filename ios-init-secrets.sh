...
if grep -q "$APP_KEY" "$pbxproj_file" && grep -q "$APP_SECRET" "$pbxproj_file"; then
    echo "Both app secrets are already defined. No changes made."
elif ! grep -q "$APP_KEY" "$pbxproj_file" && ! grep -q "$APP_SECRET" "$pbxproj_file"; then
...
elif ! grep -q "$APP_KEY" "$pbxproj_file"; then
    # Replace the placeholder for the app key
    sed -i.bak "s/$placeholder_key/$APP_KEY/g" "$pbxproj_file"
    echo "App key has been added."
elif ! grep -q "$APP_SECRET" "$pbxproj_file"; then
